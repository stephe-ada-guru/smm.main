...
with GNAT.Sockets;
package SMM.Database_Remote.IP.Test is

   type Test_Case
     (Server_IP : Ada.Strings.Unbounded.String_Access;
   ...
   --  be no way to filter those out from Ada. So we have to specify
   --  the correct IP address to use.
   --
   --  Debug
   --  0 : normal
   --  1 : don't spawn server, don't recreate db; verbose test
   --  2 : spawn server with verbose; verbose test
