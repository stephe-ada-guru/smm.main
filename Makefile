...
install-emulator-release :
	adb -e install -r build/outputs/apk/org.stephe_leake.music_player.work-release-$(VERSION).apk

install-emulator-debug :
	adb -e install -r build/outputs/apk/org.stephe_leake.music_player.work-debug-$(VERSION).apk

install-pda-usb-release :
	adb -d install -r build/outputs/apk/org.stephe_leake.music_player.work-release-$(VERSION).apk
...
install-pda-ssh-release :
	scp -P 2222 build/outputs/apk/org.stephe_leake.music_player.work-release-$(VERSION).apk root@$(PDA_IP):/storage/sdcard0/Download/

install-pda-ssh-debug-all : all install-pda-ssh-debug
install-pda-ssh-debug : PDA_IP := 192.168.1.66
install-pda-ssh-debug :
	scp -P 2222 build/outputs/apk/org.stephe_leake.music_player.work-debug-$(VERSION).apk root@$(PDA_IP):/storage/sdcard0/Download/
...
install-playlist :
	...
	adb -e push "test/Audio/vocal/David Wilcox/Into the Mystery/AlbumArt_Large.jpg" "/storage/sdcard/Audio/vocal/David Wilcox/Into the Mystery/AlbumArt_Large.jpg"
